-- Migration: 010_reset_and_seed.sql
-- WARNING: This script will WIPE all data from the tables below and re-seed them.
-- Use this for resetting the demo environment.

-- 1. Wipe existing data (CASCADE handles foreign keys)
-- Order matters less with CASCADE, but let's be logical
...
-- TRUNCATE TABLE requests CASCADE; -- Uncomment if you want to wipe requests too (existing data)
-- TRUNCATE TABLE cases CASCADE;    -- Uncomment if you want to wipe cases too (existing data)

-- Wipe clients last (or first, CASCADE handles it, but let's be explicit)
TRUNCATE TABLE clients CASCADE;


-- 2. Insert validated seed data (from 009_seed_data.sql)
